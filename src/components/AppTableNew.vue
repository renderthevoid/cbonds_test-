<template>
    <div :class="[table_class, {partTable:arrow_adaptive}, {leftColumn: arrow_screen}]" class="table_cntrl2"
         ref="tableCntrl">
        <div class="innerTableControl">
            <div class="fixedHeadTable" v-show="fixedThead">
                                ...
                                <div :class="{for_number: number}">
                                    ...
                                    <a v-if="sort_fields.indexOf(columns[0]) != -1" href="javascript:void(0);"
                                       ...
                                       v-on:click="sortByField(columns[0])">
                                        <slot name="header" :field="columns[0]" :value="header[columns[0]]"
                                              :row="header"><span
                ...
                <div ref="fixedHeadTableRight" class="fixedHeadTableRight" :style="{width: widthFixedHeadTableRight}">
                    ...
                    <table class="table" :style="{'margin-left': indent + 'px'}" border="0" cellpadding="0"
                           cellspacing="0">
                        <thead>
                        <tr>
                            ...
                            <th>
                                <div :style="{width: forFixedThF}">
                                    <!--<span v-if="number" class="number">№</span>-->
                                    <a :id="'cb_' + table_name + 'fixhead_name'"
                                       v-if="sort_fields.indexOf(columns[0]) != -1"
                                       href="javascript:void(0);" class="sortable" v-on:click="sortByField(columns[0])">
                                        <slot name="header" :field="columns[0]" :value="header[columns[0]]"
                                              :row="header"><span
                                            v-html="header[columns[0]]"></span></slot>
                                        <span v-if="columns[0] == sort_field && sort_order == 'asc'">▲</span>
                                    ...
                                    <slot v-else name="header" :field="columns[0]" :value="header[columns[0]]"
                                          :row="header"><span
                                        :id="'cb_' + table_name + 'fixhead_name'" v-html="header[columns[0]]"></span>
                                    </slot>
                                    <span class="table_header_cross"
                                          v-if="checkIfNeedToAddCross && needToAddCrossToMainColumn"
                                          @click="hideColumn(columns[0])" v-html="labelToHideColumn(columns[0])"></span>
                                </div>
                            </th>
                            <template v-for="(key, index) in columns">
                                <th v-if="index > 0" :class="['field_' + key]" :key="key">
                                    <div :style="{width: forFixedTh[index]}">
                                        <a :id="'cb_' + table_name + 'fixhead_item_' + index"
                                           v-if="sort_fields.indexOf(key) != -1"
                                           href="javascript:void(0);" class="sortable" v-on:click="sortByField(key)">
                                        ...
                                        <slot v-else name="header" :field="key" :value="header[key]" :row="header"><span
                                            ...
                                            v-html="header[key]"></span></slot>
                                        <span class="table_header_cross" v-if="checkIfNeedToAddCross"
                                              @click="hideColumn(key)"
                                              v-html="labelToHideColumn(key)"></span>
                                    </div>
                                </th>
                            </template>
            ...
            <div ref="theadLeftBlock" class="table_adaptive" v-if="arrow_screen || arrow_adaptive"
                 v-bind:style="{width: widthScreen}">
                <table class="table full" border="0" cellpadding="0" cellspacing="0">
                    <thead>
                    <tr ref="trFixedHead">
                        <th :id="'cb_' + table_name + 'fixleft_first'">
                            <div :style="{height: heightH}"
                                 :class="{for_number: number, displayBlock : (checkIfNeedToAddCross && needToAddCrossToMainColumn)}">
                                <label v-if="checkbox" class="item checkbox"></label>
                                <span v-if="number" class="number">№</span>
                                <a :id="'cb_' + table_name + '_first'" v-if="sort_fields.indexOf(columns[0]) != -1"
                                   href="javascript:void(0);" class="sortable" v-on:click="sortByField(columns[0])">
                                    <slot name="header" :field="columns[0]" :value="header[columns[0]]"
                                          :row="header"><span
                                        v-html="header[columns[0]]"></span></slot>
                                ...
                                <slot v-else name="header" :field="columns[0]" :value="header[columns[0]]"
                                      :row="header"><span
                                    :id="'cb_' + table_name + '_first'" v-html="header[columns[0]]"></span></slot>
                                <span class="table_header_cross"
                                      v-if="checkIfNeedToAddCross && needToAddCrossToMainColumn"
                                      @click="hideColumn(columns[0])" v-html="labelToHideColumn(columns[0])"></span>
                            </div>
                    ...
                    <tr v-if="prev" class="openOther" ref="trFixedBodyGroupPr">
                        <td>
                            <div v-bind:style="{height: heightBGNx}">
                                <a :id="'cb_' + table_name + '_prev'" href="javascript:void(0);" v-on:click="showPrev"
                    ...
                    <template v-for="(item, index) in body">
                        <template v-if="index < innerLimit">
                            <tr :key="'trFixedBodyPrestrings' + index"
                                :class="[item.class, getOdd(index), {click_color: activeClickRow == item ? true : false}]"
                                ...
                                <td :class="{group_tr:collapse}">
                                    <div class="small_txt" v-if="collapse">
                                        <a :id="'cb_' + table_name + '_fixed_item_' + '_' + index"
                                           href="javascript:void(0);"
                                           class="group show_hide_block" v-on:click.prevent="switchGroupList(index)"
                                           v-html="prestrings[index]"></a>
                                    </div>
                                    <div class="small_txt" v-else
                                         :id="'cb_' + table_name + '_fixed_item_' + '_' + index"
                                         v-html="prestrings[index]"></div>
                                </td>
                            </tr>
                            <tr :id="table_name + '_fixed_item_' + index" :key="'fixed_' + index"
                                :class="[item.class, getOdd(index), {click_color: activeClickRow == item ? true : false}]"
                                ref="trFixedBody"
                                v-show="showTr(item.show) && ((collapse) ? item.show_group : true) && !item.data.hidden"
                                ...
                                <td>
                                    <div :style="{height: heightBody[index]}">
                                        ...
                                        <a :id="'cb_' + table_name + '_fixed_item_' + index"
                                           v-if="typeof(item.data[columns[0]]) === 'object' && !!item.data[columns[0]] && item.data[columns[0]].lnk"
                                           :href="item.data[columns[0]].lnk" target="_blank"
                                           v-html="item.data[columns[0]].ttl"></a>
                                        <template v-else>
                                            ...
                                            <template v-else>
                                                <slot
                                                    ...
                                                    name="field"
                                                    :field="columns[0]" :value="item.data[columns[0]]" :row="item.data"><span
                                                    :id="'cb_' + table_name + '_fixed_item_' + index"
                                                    v-html="item.data[columns[0]].ttl"></span>
                                                </slot>
                                                <slot v-else name="field" :field="columns[0]"
                                                      :value="item.data[columns[0]]" :row="item.data">
                                                    <span :id="'cb_' + table_name + '_fixed_item_' + index"
                                                          v-html="item.data[columns[0]]"></span>
                                                </slot>
                                            </template>
                                        </template>
                    ...
                    <tr v-if="next" class="openOther" ref="trFixedBodyGroupNx">
                        <td>
                            <div v-bind:style="{height: heightBGNx}">
                                <a :id="'cb_' + table_name + '_next'" href="javascript:void(0);" v-on:click="showNext"
            ...
            <a href="javascript:void(0);" class="arrow_right_screen" v-if="arrow_screen && arrow_screen_right"
               :class="{arrow_right_screen_multy:arrow_multy}" v-on:mousedown="startScrollR"
               v-on:mouseleave="stopScroll"
               v-on:mouseup="stopScroll" v-on:touchstart="startScrollR" v-on:touchend="stopScroll"></a>
            <a href="javascript:void(0);" class="arrow_left_screen" v-if="arrow_screen && arrow_screen_left"
               :class="{arrow_left_screen_multy:arrow_multy}" v-on:mousedown="startScrollL" v-on:mouseleave="stopScroll"
               v-on:mouseup="stopScroll" v-on:touchstart="startScrollL" v-on:touchend="stopScroll"></a>
            <a href="javascript:void(0);" class="arrow_left" v-if="arrow_adaptive && index > 1"
               v-on:click="scrollTableLeft"></a>
            <a href="javascript:void(0);" class="arrow_right" v-if="arrow_adaptive && index < countActiveColumn"
               v-on:click="scrollTableRight"></a>
            <!--Прокручивающаяся таблица-->
            <div class="table_wrapper" ref="table_wrapper">
                <table ref="table_scroll" class="table full" :class="{indent:arrow_screen}"
                       :style="{width: widthFull, 'margin-left': indent + 'px'}" border="0" cellpadding="0"
                       cellspacing="0">
                    <thead>
                    <tr ref="trScrollHead">
                        <th :id="'cb_' + table_name + '_checkbox'" ref="width_th_scroll_checkbox"
                            v-if="checkbox && (!arrow_adaptive && !arrow_screen)"></th>
                        ...
                        <th :id="'cb_' + table_name + '_first'" ref="width_th_scroll_first">
                            <div :style="{width: width}">
                                ...
                                <span class="table_header_cross" v-if="checkIfNeedToAddCross"
                                      ...
                                      v-html="labelToHideColumn(columns[0])"></span>
                            </div>
                        </th>
                        <template v-for="(key, index) in columns">
                            <th v-if="index > 0" :id="'cb_' + table_name + '_scroll_head_' + index"
                                ref="width_th_scroll"
                                :class="['field_' + key]" :key="key">
                                <div :style="{width: width}">
                        ...
                        <th class="field_screen_text" ref="width_th_scroll_group"
                    ...
                    <tr v-if="prev" class="openOther">
                        <td :colspan="colspan" ref="trFixedBodyGroupNx">
                            <div>
                                <a :id="'cb_' + table_name + '_show_prev'" href="javascript:void(0);"
                                   v-on:click="showPrev"
                                   :class="{show_hide_block:prevInner, hide_hide_block:!prevInner}">
                                    <template v-if="!prevInner">{{ getLabel('link_show_prevs') }}</template>
                                    <template v-if="prevInner">{{ getLabel('hide') }}</template>
                    ...
                    <template v-for="(item, index) in body">
                        <template v-if="index < innerLimit">
                            <tr :key="index"
                                :class="[item.class, getOdd(index), {click_color: activeClickRow == item ? true : false}]"
                                v-if="prestrings[index]" v-on:click="setActiveColor(item)">
                                <td
                            ...
                            <tr :id="table_name + '_scroll_item_' + index" :key="'trScrollBody' + index"
                                ...
                                v-show="showTr(item.show) && ((collapse) ? item.show_group : true) && !item.data.hidden"
                                v-on:click="setActiveColor(item)">
                                <td :id="'cb_' + table_name + '_checkbox_' + index"
                                ...
                                <td :id="'cb_' + table_name + '_scroll_item_first_' + index">
                                    <div :style="{width: width}">
                                        ...
                                        <template
                                            v-if="!!item.data[columns[0]] && typeof(item.data[columns[0]]) === 'object'">
                                            <span v-if="number" class="number">{{ index + 1 }}</span>
                                            <a v-if="item.data[columns[0]].lnk" :href="item.data[columns[0]].lnk"
                                               target="_blank"
                                               v-html="item.data[columns[0]].ttl"></a>
                                            <template v-else>
                                                ...
                                                <slot v-else name="field" :field="columns[0]"
                                                      ...
                                                      :row="item.data"><span v-html="item.data[columns[0]].ttl"></span>
                                                </slot>
                                            </template>
                                            <div class="small" v-if="item.data[columns[0]].info"
                                                 v-html="item.data[columns[0]].info"></div>
                                        ...
                                        <template v-else>
                                            ...
                                            <slot v-else name="field" :field="columns[0]" :value="item.data[columns[0]]"
                                                  :row="item.data">
                                                <span v-html="item.data[columns[0]]"></span></slot>
                                        </template>
                                    </div>
                                </td>
                                <template v-for="(key, indexInner) in columns">
                                    <td v-if="indexInner > 0"
                                        ...
                                        <div :style="{width: width}" :title="getTitle(item.data[key])">
                                            <template v-if="!!item.data[key] && typeof(item.data[key]) === 'object'">
                                                <a v-if="item.data[key].lnk" :href="item.data[key].lnk" target="_blank"
                                                ...
                                                <template v-else>
                                                    ...
                                                    <slot v-else name="field" :field="key" :value="item.data[key].ttl"
                                                          :row="item.data"><span
                                                        v-html="item.data[key].ttl"></span></slot>
                                                </template>
                                ...
                                <template v-if="group && (arrow_adaptive || arrow_screen) && substrings.length > 0">
                            ...
                            <tr :class="[item.class, getOdd(index), {click_color: activeClickRow == item ? true : false}]"
                            ...
                            >
                                <td v-if="getAsterisks(substrings[index])"
                                    :id="'cb_' + table_name + '_scroll_prestrings_' + index"
                                    :colspan="colspan" class="small_txt">
                                    ***
                                </td>
                                <td v-else :id="'cb_' + table_name + '_scroll_prestrings_' + index" :colspan="colspan"
                                    class="small_txt"
                                    v-html="substrings[index]"></td>
                            </tr>
                        </template>
                    </template>
                    ...
                    <tr v-if="next" class="openOther">
                        <td :colspan="colspan">
                            <div>
                                <a :id="'cb_' + table_name + '_show_next'" href="javascript:void(0);"
                                   v-on:click="showNext"
                                   :class="{show_hide_block:!nextInner, hide_hide_block:nextInner}">
                                    <template v-if="!nextInner">{{ getLabel('link_show_nexts') }}</template>
                                    <template v-if="nextInner">{{ getLabel('hide') }}</template>
        ...
        <div class="top_20" v-if="limit > 0 && limit < data.length" v-cloak>
            <a :id="'cb_' + table_name + '_show'" href="javascript:void(0);"
               :class="[show_tr ? 'hide_hide_block' : 'show_hide_block']" v-on:click.prevent="switchTr">
        ...
        <div class="top_10" v-if="data.length > 0 && btnShowHidden" v-cloak>
            ...
            <a :id="'cb_' + table_name + '_show'" href="javascript:void(0);" v-on:click="switchTr2"
        ...
        <div class="fullDescrTableMobile" v-if="showFullDescrTableMobile">
            <a href="javascript:void(0);" class="close"
               v-on:click.prevent="showFullDescrTableMobile=false">{{ label_hide }}</a>
...
export default {
        ...
        data: {
            handler() {
                if (!(this.show_tr && this.data.filter((row) => !row.hidden).length == this.innerLimit)) {
                    this.innerLimit = this.limit > 0 ? this.limit : this.data.filter((row) => !row.hidden).length;
                    this.show_tr = false;
                }
            },
            deep: true,
    ...
    computed: {
        label_show_more() {
            return typeof (this.$store.state.labels.show_more) != 'undefined' ? this.$store.state.labels.show_more : 'Show more'
        },
        label_hide() {
            return typeof (this.$store.state.labels.hide) != 'undefined' ? this.$store.state.labels.hide : 'Hide'
        },
        ...
        body: function () {
            ...
            if (self.data_ext.length) {
                ...
                dataExt = self.data.filter((row) => !row.hidden).map(function (row) {
                    let rowExt = {data: row};
                    if (row.class) {
    ...
    methods: {
        ...
        getWindowWidth: function (scroll = true) {
            ...
            this.$nextTick(function () {
                ...
                self.forFixedThN = self.$refs.width_th_scroll_number ? self.$refs.width_th_scroll_number.clientWidth - 10 + 'px' : '';
                self.forFixedThF = self.$refs.width_th_scroll_first ? self.$refs.width_th_scroll_first.clientWidth - 10 + 'px' : '';
                self.forFixedThG = self.$refs.width_th_scroll_group ? self.$refs.width_th_scroll_group.clientWidth - 10 + 'px' : '';
                self.heightBodyGroupNx = self.$refs.trFixedBodyGroupNx ? self.$refs.trFixedBodyGroupNx.clientHeight - 18 + 'px' : '';
                ...
                if (self.$refs.table_scroll.clientWidth > self.$refs.tableCntrl.clientWidth) {
                    ...
                    self.arrow_screen = self.$refs.table_scroll.clientWidth > self.$refs.table_wrapper.clientWidth;
